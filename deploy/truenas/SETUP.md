...
| `/health` says `schema incomplete: missing ...` | The init script aborted partway, so some tables were never created. `schema.sql` is idempotent — re-apply it, no wipe needed: `docker exec -i ix-fitness-sidecar-db-1 psql -v ON_ERROR_STOP=1 -U fitness -d exercise_intel < sidecar/schema.sql` |
| Profile saves, then every page 500s with `relation "trainee_benchmarks" does not exist` | Same cause. `get_profile` only reads that table once a profile row exists, so a partial schema stays hidden until the first save. Re-apply `schema.sql` as above. |
